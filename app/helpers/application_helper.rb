module ApplicationHelper
  ...
  def transaction_url(txid)
    "http://gamers-coin.org:3000/gmc/tx/#{txid}"
  end
  ...
  def address_url(address, explorer = address_explorers.first)
    case explorer
    when :blockr then "http://gamers-coin.org:3000/gmc/address/#{address}"
    when :bkchain then "http://gamers-coin.org:3000/gmc/address/#{address}"
    when :cryptocoin then "http://gamers-coin.org:3000/gmc/address/#{address}"
