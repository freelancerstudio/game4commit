class UsersController < ApplicationController
  ...
  def update
    if @user.update(users_params)
      ...
      render :show, alert: 'Error updating gamerscoin address'
    end
  end
  ...
  def login
    ...
    if @user
      if params[:unsubscribe]
        ...
        flash[:alert] = 'You unsubscribed! Sorry for bothering you. Although, you still can leave us your gamerscoin address to get your tips.'
      end
  ...
  def set_password_and_address
    ...
    if @user.confirmed?
      redirect_to new_session_path(User), notice: "Your account is already confirmed. Please sign in to set your Gamerscoin address."
      return
    end
